...
func NewExpr(preds ...interface{}) WriterTo {
	...
	case WriterTo:
		...
		msg := fmt.Sprintf("common/sql: NewExpr only accepts single WriterTo")
		panic(msg)
	default:
		msg := fmt.Sprintf("common/sql: unsupported type (got %T)", pred)
		panic(msg)
	}
}
		...
		for _, arg := range args {
			if p, ok := arg.(WriterTo); ok {
				*ps = append(*ps, p)
				...
				msg := fmt.Sprintf("common/sql: all args must implement WriterTo interface (got %T)", arg)
				panic(msg)
			}
		}
	default:
